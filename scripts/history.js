...
function update_elements() {
    ...
    for (var trans of currUser.transactions) {
        ...
        $(".wrapper").html($(".wrapper").html() + '<div class="transaction frosted white-1"><span class="amount ' + color + '-1">' + sign + "₹" + amount + "</span><i data-lucide='circle-user-round'></i><div><span class='name'>" + title + "</span></div><span class='date'>" + date + "</span></div>");
    }
    if (x == 0) {
        $(".wrapper").html($(".wrapper").html() + "<h1>Add Transactions to begin...</h1>");
    }
    $(".loader").fadeOut();
    main();
}
